...
public sealed class FileServiceTests : IDisposable {
 ...
 var table=service.ReadSite(P("a.csv"),new());
 ...
 [Theory] [InlineData("sku;status\n001;\"broken")] [InlineData("sku;status\n001;ok;extra")] [InlineData("sku,status;other\n001,ok;x")] public void RejectsInvalidOrAmbiguousCsv(string data){File.WriteAllText(P("a.csv"),data);Assert.Throws<InvalidDataException>(()=>service.ReadSite(P("a.csv"),new()));}
 [Fact] public void ExportRoundTripAndAtomicFailure(){var table=new TableData(["sku","status","extra"],new[]{new[]{"001","Новинка","😀"}});service.Export(table,P("out.csv"),new(){SkuHeader="код",StatusHeader="стан"},[]);var reread=service.ReadSite(P("out.csv"),new());Assert.Equal(new[]{"код","стан","extra"},reread.Headers);Assert.Equal(table.Rows[0],reread.Rows[0]);var before=File.ReadAllBytes(P("out.csv"));Assert.Throws<InvalidDataException>(()=>service.Export(table,P("out.csv"),new(){EncodingName="windows-1251"},[]));Assert.Equal(before,File.ReadAllBytes(P("out.csv")));Assert.Throws<InvalidDataException>(()=>service.Export(table,P("out.csv"),new(),[P("out.csv")]));}
