...
export function ImagesModule() {
  ...
  async function removeSelected() {
    const ids=[...selected];
    const response=await exec('removeSelected',{ids});
    if(response?.failed)return;
    setSelected(new Set());
    if(selected.has(active)){setActive('');setSource('');setPreview('');setPreviewOptions('');}
    toast.success(`Видалено з галереї: ${ids.length}`);
  }
  ...
  return <div className="space-y-3" data-testid="images-module">
    ...
    <Tabs value={tab} onValueChange={setTab}><TabsList><TabsTrigger value="load">Завантаження</TabsTrigger><TabsTrigger value="edit">Зображення ({state.items.length})</TabsTrigger><TabsTrigger value="settings">Налаштування</TabsTrigger><TabsTrigger value="warnings">Попередження ({state.warnings.length})</TabsTrigger><TabsTrigger value="help">Як користуватися</TabsTrigger></TabsList>
      ...
      <TabsContent value="edit"><div className="space-y-3">{!state.items.length?<Panel><EmptyState title="Додайте зображення" hint={state.message}/></Panel>:<>
        <Panel><div className="mb-3 flex flex-wrap items-center gap-3"><label className="flex items-center gap-2 text-xs"><input type="checkbox" disabled={busy} checked={state.items.length>0&&selected.size===state.items.length} onChange={e=>setSelected(e.target.checked?new Set(state.items.map(x=>x.id)):new Set())}/>Обрати всі</label><span className="text-xs text-muted-foreground">Натисніть фото для редагування. Обрано: {selected.size}</span><Button size="sm" variant="secondary" disabled={busy||!selected.size} title="Прибрати з галереї. Файли на диску залишаться." onClick={()=>void removeSelected()}>Видалити обрані ({selected.size})</Button></div><div className="flex max-h-44 gap-3 overflow-auto">{state.items.map(photo=><div key={photo.id} className={`w-28 shrink-0 rounded-md border p-2 ${active===photo.id?'border-primary ring-1 ring-primary':'border-border'}`}><button disabled={busy} onClick={()=>void open(photo.id)} className="block w-full"><img src={photo.thumbnail} alt={photo.name} className="h-12 w-full object-contain"/></button><label className="mt-1 flex items-center gap-1 text-[10px]"><input type="checkbox" disabled={busy} checked={selected.has(photo.id)} onChange={e=>setSelected(old=>{const next=new Set(old);e.target.checked?next.add(photo.id):next.delete(photo.id);return next;})}/><span className="truncate" title={photo.name}>{photo.name}</span></label><div className="text-[10px] text-muted-foreground">{photo.width} × {photo.height}{photo.saved?' · ✓':''}</div></div>)}</div></Panel>
