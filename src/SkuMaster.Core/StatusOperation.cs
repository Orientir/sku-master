        ...
        CancellationToken cancellationToken = default)
    {
        ...
        for (var i = 0; i < input.Rows.Count; i++)
        {
            ...
            if (original.Length != input.Headers.Length)
                throw new ArgumentException($"Експорт сайту, рядок {i + 2}: кількість колонок не відповідає заголовкам.");
            var row = (string[])original.Clone();
            rows.Add(row);
            var sku = row[skuIndex].Trim();
            if (sku.Length == 0)
            {
                warnings.Add($"Експорт сайту, рядок {i + 2}: порожній SKU; статус залишено без змін.");
                continue;
            }
            if (!seen.Add(sku)) warnings.Add($"Експорт сайту, рядок {i + 2}: повторний SKU «{sku}».");
            ...
            changes.Add(new(i + 2, row[skuIndex], oldStatus, newStatus,
                found ? ChangeKind.Available : ChangeKind.Unavailable));
        }
        return new(new((string[])input.Headers.Clone(), rows, input.Delimiter, input.EncodingName), changes, warnings, seen.Count);
    }
